 ...
 * 2022-03-31
 * Modified by: @WYFANG <https://github.com/wyfang>
 * Modifications: UI redesign, watermark logic update, 9-grid selector, drag-and-drop support.
 */
...
const patina = (imageEl, _config, app)=>{
    ...
    requestAnimationFrame(_=>{
        ...
        const watermark = _=>{
            // 1. 找出所有被激活的格子索引 (0-8)
            // locations: [true, false, true, ...]
            const activeIndices = [];
            if (_config.watermarkLocations) {
                _config.watermarkLocations.forEach((isActive, idx) => {
                    if(isActive) activeIndices.push(idx);
                });
            } else {
                // 兼容旧配置：默认全选
                for(let k=0; k<9; k++) activeIndices.push(k);
            }
            ...
            // 2. 随机挑一个位置
            const posIndex = activeIndices[randRange2(0, activeIndices.length - 1)];
            
            // 3. 计算行列 (3x3 grid)
            // 0 1 2
            // 3 4 5
            // 6 7 8
            const row = Math.floor(posIndex / 3); // 0=Top, 1=Mid, 2=Bot
            const col = posIndex % 3;             // 0=Left, 1=Cen, 2=Right
            ...
            // 4. 计算坐标和对齐方式
            const padding = fontSize; // 留点边距
            let x, y;
            
            // 水平方向
            if (col === 0) { // Left
                ctx.textAlign = 'left';
                x = padding + randRange(-5, 5);
            } else if (col === 1) { // Center
                ctx.textAlign = 'center';
                x = _width / 2 + randRange(-10, 10);
            } else { // Right
                ctx.textAlign = 'right';
                x = _width - padding + randRange(-5, 5);
            }

            // 垂直方向
            if (row === 0) { // Top
                ctx.textBaseline = 'top';
                y = padding + randRange(-5, 5);
            } else if (row === 1) { // Middle
                ctx.textBaseline = 'middle';
                y = _height / 2 + randRange(-5, 5);
            } else { // Bottom
                ctx.textBaseline = 'bottom';
                y = _height - padding + randRange(-5, 5);
            }

            ctx.fillText('@'+randName(_config.userNames), x, y);
        }
        
        // 初始第一层水印，建议保留，否则第一层可能没水印
        if(_config.watermark && Math.random() * 100 <= (_config.watermarkDensity === undefined ? 100 : _config.watermarkDensity)){
            watermark();
        }
